 ...
 */
public class FastdexTerminal implements FocusListener, ProjectComponent {

    private JBTabbedTerminalWidget myTerminalWidget;
    private Project myProject;

    public FastdexTerminal(Project project) {
        this.myProject = project;
    }

    public static FastdexTerminal getInstance(Project project) {
        return project.getComponent(FastdexTerminal.class);
    }
    ...
    public JBTabbedTerminalWidget getTerminalWidget(ToolWindow window) {
        ...
        if (myTerminalWidget == null) {
            ...
            if (parentPanel instanceof SimpleToolWindowPanel) {
                ...
                NotificationUtils.infoNotification("Wait for Fastdex to initialize");
            }
        }
    ...
    public void initAndExecute(final ArrayList<String> shell) {
       initAndExecute(null,shell);
    }

    public void initAndExecute(String workDir,final ArrayList<String> shell) {
        String[] array = new String[shell.size()];
        for (int i = 0; i < shell.size(); i++) {
            array[i] = shell.get(i);
        }
        initAndExecute(workDir,array);
    }
    ...
    private ToolWindow getToolWindow() {
        return ToolWindowManager.getInstance(myProject).getToolWindow(FastdexToolWindowFactory.TOOL_WINDOW_ID);
    }
    ...
    private static class StopAction extends BaseTerminalAction {
        ...
        public StopAction(FastdexTerminal terminal) {
            super(terminal, "Stop Run Fastdex", "Stop Run Fastdex", PluginIcons.Suspend);
            try {
                robot = new Robot();
    ...
    private static class RunAction extends BaseTerminalAction {
        public RunAction(FastdexTerminal terminal) {
            this(terminal, "Run Fastdex", "Run Fastdex", PluginIcons.FastdexIcon);
        }

        public RunAction(FastdexTerminal terminal, String text, String description, Icon icon) {
    ...
    private static class ClearAction extends BaseTerminalAction {
        public ClearAction(FastdexTerminal terminal) {
    ...
    private static abstract class BaseTerminalAction extends DumbAwareAction {
        protected FastdexTerminal terminal;

        public BaseTerminalAction(FastdexTerminal terminal, String text, String description, Icon icon) {
        ...
        public void actionPerformed(AnActionEvent anActionEvent) {
            ...
            FastdexStatus status = FastdexUtil.checkInstall(anActionEvent.getProject());
            if (status != null) {
